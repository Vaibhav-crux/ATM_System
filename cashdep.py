...
class CashDeposite:
    ...
    def acc_no(self,app):
        #global acc_entry
        #LABEL CASH WITHDRAW
        self.comm_label= Label(app,text="Cash Deposite",font=("Arial",30),bg="#3BB9FF",fg="White").grid(row=2,column=0,pady=10)
    ...
    def acc_balance(self,app):
        self.sql_data = self.cursor.execute(f"select USER_PIN from generate_account where USER_ACCOUNT={self.acc_data.get()}")  # GETTING ACCOUNT NUMBER FROM SQL DATABASE
        self.sql_pin_value = self.cursor.fetchall()  # FR=FETCHING VALUE FROM SQL
        self.sql_pin_conv=""
        for i in self.sql_pin_value:
            self.sql_pin_conv+=str(i)

        if(str(self.pin_data.get()) in self.sql_pin_conv):
            self.pin_entry.config(state="disabled")
            self.pin_button.config(state="disabled")
            self.acc_bal_label=Label(app,text="ENTER AMOUNT",bg="#3BB9FF",font=('Arial',20,'bold'),fg="blue").grid(row=9,column=0,pady=5)
        #BALANACE ENTRY
            self.bal_data=StringVar()
            self.acc_bal_enter=Entry(app,textvariable=self.bal_data,bd=6,width=10,font=("bold"))
            self.reg_bal_entry=app.register(self.fun_bal_entry)
            self.acc_bal_enter.config(validate="key",validatecommand=(self.reg_bal_entry,'%P'))
        #BALANCE BUTTON
            self.acc_bal_button=Button(app,command=lambda :[self.cash_deposite_exit(app)],text="DEPOSITE CASH",bg="#3BB9FF",fg="White",width=20)
            self.acc_bal_enter.grid(row=10,column=0,pady=5)
            self.acc_bal_button.grid(row=11,column=0)
        else:
    ...
    def cash_deposite_exit(self,app):
        ...
        if self.acc_data.get() == "" or self.bal_data.get() == "":
            messagebox.showwarning('ERROR',"Enter Amount")
            
        else:
            if(int(self.bal_data.get())>0):
                self.t = time.localtime()
                self.ct = time.strftime("%H:%M:%S", self.t)
                self.d = date.today()
                self.t = time.localtime()
                self.ct = time.strftime("%H:%M:%S", self.t)
                self.sql_data="INSERT INTO cash_deposite(USER_ACCOUNT,USER_DEPOSITE,DATE,TIME) VALUES (%s,%s,%s,%s)"
                self.sql_val=(self.acc_data.get(),self.bal_data.get(),self.d,self.ct)
                self.cursor.execute(self.sql_data,self.sql_val)
                self.my_db.commit()
                messagebox.showinfo("DONE", "Transaction Complete")
                self.app.destroy()
                self.deposite_transaction()

            else:
                messagebox.showwarning('INVALID',"0 Balance Can't deposite")
                app.destroy()

    def deposite_transaction(self):

        self.sql_acc_check = self.cursor.execute(f"select USER_ACCOUNT from amount where USER_ACCOUNT={self.acc_data.get()}")
        self.sql_acc_value = self.cursor.fetchall()
        self.sql_acc_conv = ""
        for i in self.sql_acc_value:
            self.sql_acc_conv += str(i)  # (77777777777,)
        self.pre_bal = self.sql_acc_conv[2:12]
        ...
        if (self.acc_data.get() in self.sql_acc_conv):
            ...
            self.total_amount = int(self.sql_prev_bal) + int(self.bal_data.get())
