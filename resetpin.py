...
class ResetPIN:
    ...
    def acc_no(self,app):
        ...
        #LABEL CASH WITHDRAW
        global acc_data
        comm_label= Label(app,text="Reset PIN",font=("Arial",30),bg="#3BB9FF",fg="White").grid(row=2,column=0,pady=10)
        ...
        store_acc_data=acc_data.get()
        new_pin_data=""
        acc_sub_but = Button(app,command=lambda :[ser_pin(app)],text="SUBMIT", bg="#3BB9FF", fg="white", width=20, state="disabled")#CREATING ACCOUNT SUBMIT BUTTON
    ...
    def user_pin(self,app):
        ...
        if(str(acc_data.get()) in sql_data_conv):
            ...
            pin_button=Button(app,text="Submit", command=lambda :[reset_pin(app)],bg="#3BB9FF", fg="white", width=20, state="disabled")
    ...
    def reset_pin(self,app):
        sql_pin_verify=cursor.execute(f"select USER_PIN from generate_account where USER_ACCOUNT={acc_data.get()}")
        sql_pin_data=cursor.fetchall()
        sql_pin_data_conv=(sql_pin_data[0][0])
        if(pin_data.get()==.sql_pin_data_conv):
            '''import resetpinconf as rpc
            root = Toplevel()
            cw = rpc.OldPinReset(root)
            cw.check_pin(acc_data)
            root.mainloop()'''
